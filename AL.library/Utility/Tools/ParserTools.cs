...
namespace AL.library.Utility.Tools
{
    public static class ParserTools
    {
        ...
        public static DateTime? ToDateTimeNullable(object value, DateTime? defaultValue)
        {
            ...
            if (DateTime.TryParse(value.ToString(), CultureInfo.GetCultureInfo("pt-br"), DateTimeStyles.None, out ret))
                return ret;

            if (DateTime.TryParse(value.ToString(), CultureInfo.InvariantCulture, DateTimeStyles.None, out ret))
                return ret;
        ...
        public static string ToHexString(this string str)
        {
            ...
            for (var i = 0; i < str.Length; i++)
                returnValue.Append(string.Format("{0:X2}", Convert.ToInt32(str[i])));
